...
def generate_train_test_split(
    ...
    for fileName in inputFiles:
        ...
        ind = ind / ind.std(axis=0)
        _t = np.empty((t.shape[0], t.shape[1] + 1))
        _t[:, :-1] = t
        _t[:, -1] = 1. - np.sum(t, axis=1)
        dataStep.append((ind, _t))

    dataStep = shuffleDataStep(
        dataStep, nFolds=nFolds, nRepeat=nRepeat, shuffle=shuffle
    )

    n_train_samples = int(len(dataStep) * train_split)

    train_data = []
    for ind, t in dataStep[:n_train_samples]:
        train_data.append((ind, t))

    test_data = []
    for ind, t in dataStep[n_train_samples:]:
        test_data.append((ind, t))
...
class UniHHIMUGestures(TorchDataset):
    def __init__(
        ...
        if isinstance(data, tuple):
            ...
            for fileName in inputFiles:
                ind, t = createData(
                    fileName, inputGestures, usedGestures, dataDir=dataDir
                )
                ind = ind / ind.std(axis=0)
...
class DataSet(object):
    ...
    def getDataForTraining(
        self, classNrs, targetNr=2, multiplier=1, normalized=False, power=False
    ):
            ...
            for ind in changesT:
                if target[lastInd] == 1:
                    signals.append(
                        np.concatenate(
                            (
                                self.fused[lastInd:ind, :],
                                self.gyro[lastInd:ind, :],
                                self.acc[lastInd:ind, :],
                                np.atleast_2d(target[lastInd:ind]).T,
                            ),
                            1,
                        )
                    )
    ...
    def writeToFile(self, fileName):
        np.savez(
            getProjectPath() + "dataSets/" + fileName,
            fused=self.fused,
            gyro=self.gyro,
            acc=self.acc,
...
def appendDS(dataSets, usedGestures):
    result = (
        dataSets[0].getDataForTraining(usedGestures, 2)[0],
        dataSets[0].getDataForTraining(usedGestures, 2)[1],
    )
    for i in range(1, len(dataSets)):
        result = (
            np.append(result[0], dataSets[i].getDataForTraining(usedGestures, 2)[0], 0),
            np.append(result[1], dataSets[i].getDataForTraining(usedGestures, 2)[1], 0),
        )
    return result


def createData(
    dataSetName, inputGestures, usedGestures, scaleFactor=1, dataDir="dataSets/"
):
    dataSets = []
    ...
    if scaleFactor != 1:
        scaledInputs = np.zeros(
            (resultInputs.shape[0] * scaleFactor, resultInputs.shape[1])
        )
        scaledTargets = np.zeros(
            (resultTargets.shape[0] * scaleFactor, resultTargets.shape[1])
        )
